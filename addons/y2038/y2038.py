...
def reportDiagnostic(configuration, location, category, message):
...
def reportDirDiag(cfg, filename, linenr, directive, severity, msg):
    reportDiagnostic(cfg.name,
                     directive.file + ':' + directive.linenr,
                     severity, msg)
    if (filename != directive.file) or (linenr != directive.linenr):
        reportDiagnostic(cfg.name,
                         filename + ':' + linenr,
                         'Note', directive.file + ' was included from here')


def reportTokDiag(cfg, token, severity, msg):
    reportDiagnostic(cfg.name,
...
re_define_time_bits_64 = re.compile('^\s*#\s*define\s+_TIME_BITS\s+64\s*$')

# test for '#define _TIME_BITS ...' (combine w/ above to test for 'not 64')
re_define_time_bits = re.compile('^\s*#\s*define\s+_TIME_BITS\s+.*$')

# test for '#undef _TIME_BITS' (if it ever happens)
re_undef_time_bits = re.compile('^\s*#\s*undef\s+_TIME_BITS\s*$')

# test for '#define _USE_TIME_BITS64'
re_define_use_time_bits64 = re.compile('^\s*#\s*define\s+_USE_TIME_BITS64\s*$')

# test for '#undef _USE_TIME_BITS64' (if it ever happens)
re_undef_use_time_bits64 = re.compile('^\s*#\s*undef\s+_USE_TIME_BITS64\s*$')
...
for arg in sys.argv[1:]:
    # only process .dump file names
    if arg == '-q':
        quiet = True
    elif arg.endswith('.dump'):
        if not arg in dumpfiles:
            dumpfiles.append(arg)
    else:
        for (top, subdirs, files) in os.walk(arg):
            for f in files:
                if f.endswith('.dump'):
                    if not arg in dumpfiles:
                        dumpfiles.append(top + '/' + f)
dumpfiles.sort()
    ...
    for cfg in data.configurations:
        ...
        for directive in cfg.directives:
            ...
            if re_define_time_bits_64.match(directive.str):
                ...
                reportDirDiag(cfg, srcfile, srclinenr, directive, 'Error',
        ...
        for token in cfg.tokenlist:
            if token.str in id_Y2038:
                if not any(lower <= int(token.linenr) <= upper
                    ...
                    reportTokDiag(cfg, token, 'Warning',
